---
title: "Clinical Implications for Black Americans of Removing Race from CKD-EPI Estimates of Kidney Function"
...
keep_var <- list(
  ...
  ALB_CR = c("URXUMA","URXUCR","URDACT"),
  KIQ_U = c("KIQ025", "KIQ026"),
  BPX = c("BPXSY1","BPXSY2","BPXSY3","BPXDI1","BPXDI2","BPXDI3"),
  BPQ = c("BPQ030","BPQ040A"), 
  DIQ = c("DIQ010","DIQ050","DID070","DIQ160","DIQ175A"),
  GLU = c("LBXGLU"),
  GHB = c("LBXGH"),
  HIV = c("LBDHI", "LBXHIVC","LBXHIV1","LBXHIV2","LBXHNAT"),
  HEPC = c("LBDHCI","LBXHCR"),
  HEPBD = c("LBDHBG"),
  SMQRTU = c("SMDANY"),
...
if (1999 %in% cycle_years_input) {
  to_rm <- c(to_rm, "BIOPRO_A", "ALB_CR", "KIQ_U", "GLU", "GHB", "HIV", "HEPC", "SMQRTU")
  to_add <- c(to_add, "LAB18", "LAB16", "KIQ", "LAB10AM", "LAB10", "LAB03", "LAB02", "SMQMEC")
}
...
if (all(cols_match)) {
  all_data <- lapply(joined_data, function(dat) {
    ...
    dat$WTSCR2YR = dat$WTSCR2YR * us_pop_2018 / sum(dat$WTSCR2YR[!dat$MISSING])
    ...
    dat$WTSCR2YR[nm_black] <- dat$WTSCR2YR[nm_black] * black_pop_2018/obs_black_pop
    ...
    dat$WTSCR2YR[nm_nonblack] <- dat$WTSCR2YR[nm_nonblack] * 
                              (us_pop_2018 - black_pop_2018) / 
                              (obs_total_pop - black_pop_2018)
    dat$WTSCR18YR = dat$WTSCR2YR / (length(cycle_years_input)/2)
    return(dat)
...
black_adult_pop <- dat %>% 
  ...
  select(WTSCR18YR) %>% unlist %>% as.numeric %>% sum
