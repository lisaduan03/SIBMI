...
visit_level_filtered <- visit_level_first_visit %>%
...
columns_to_select <- c("PID", "TG", "TC", "HDL", "LDL", "ALANINE_AMINOTRANSFE", "SERUM_ALBUMIN", "ALKALINE_PHOSPHATASE", "ASPARTATE_AMINOTRANS", "CALCIUM", "CHLORIDE", "GLUCOSE", "HEMOGLOBIN_A1C", "POTASSIUM", "SODIUM", "TOTAL_BILIRUBIN", "TOTAL_PROTEIN", "SERUM_UREA_NITROGEN", "MCH", "MCV", "RBC", "CREATININE_SERUM", "WBC", "CBCHEMOGLOBIN", "HEMATOCRIT", "PLATELETS",  "EOSINOPHILS", "BASOPHILS", "NEUTROPHILS", "MONOCYTES", "LYMPHOCYTES", "ADJUSTED_IGFR")
...
destination <- "/Users/lisaduan/Dropbox/Mac/Desktop/SIBMI/SIBMI/eGFR-Race-master/nhanes_data"
# Download and merge NHANES data using `RNHANES` package
raw_data <- nhanes_load_data(data_file_list, year = cycle_years,
                             destination = destination, cache = TRUE,
...
variables <- setdiff(names(CRIC_data), c("PID", "ADJUSTED_IGFR"))

# plot IGFR
igfr = "ADJUSTED_IGFR"

 ggplot() +
    geom_density(data = CRIC_data, aes_string(x = igfr, fill = "'CRIC_data'"), 
                 alpha = 0.5, position = "identity") +
    labs(title = paste(igfr), x = igfr, y = "Density") +
    scale_fill_manual(name = "Dataset", values = c("CRIC_data" = "blue", "NHANES_data" = "green")) +
    theme_minimal()
