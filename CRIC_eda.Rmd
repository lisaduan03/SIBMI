...
```{r using table1 package to recreate Table 1}
...
```{r Systematic Difference: mGFR- eGFR}

data$differences = data$IGFR - data$EGFR_CKD_EPI
print(median(data$differences))

```


```{r Recreating Figure 1 quantile reg}
# 1) Perform quantile regression for the specified quantiles
# 2) use models to predict iGFR at specific eGFR values
# 3) store in a data frame 
# 4) plot results 
...
p <- ggplot(predicted_mGFR, aes(x = eGFR, y = mGFR)) +
  geom_segment(data = quantile_025, aes(x = eGFR, xend = eGFR, y = mGFR, yend = quantile_975$mGFR), color = "red") +
  geom_point(aes(shape = quantile, color = quantile), size = 3) +
  scale_shape_manual(values = shape_map) +
  scale_color_manual(values = color_map) +
  scale_y_continuous(limits = c(0, 150)) + 
  scale_x_continuous(breaks = seq(0, 120, by = 15)) +
  labs(x = "eGFR (mL/min/1.73 m^2)", y = "mGFR (mL/min/1.73 m^2)", title = "Quantile Regression: Modeling different percentiles of mGFR at given eGFR") +
  theme_minimal() +
...
# filter dataset for only laboratory
laboratory_data <- variables[variables$component == "laboratory", ]

# count number of unique years in which data_file_description occurs
unique_year_counts <- laboratory_data %>%
  group_by(data_file_description) %>%
  summarise(unique_years = n_distinct(begin_year), .groups = 'drop')
# sort in descending order
unique_year_counts_sorted <- unique_year_counts %>%
  arrange(desc(unique_years))
# only take data files with 

print(unique_year_counts_sorted)

# filter laboratory_data to only include variables with data_file_description of X
X_questionnaire_data <- laboratory_data %>%
  filter(data_file_description == "Albumin & Creatinine - Urine")
# see how many variables there are 
length(unique(X_questionnaire_data$variable_name))
list(unique(X_questionnaire_data$variable_name))


# filter variables such that anything starting with LBX is retained
lbx_variables = variables %>%
  filter(grepl("^LBX", variable_name))

# View the filtered data
print(lbx_variables)

# check for duplicate variable names
duplicate_variables <- lbx_variables %>%
  filter(duplicated(variable_name) | duplicated(variable_name, fromLast = TRUE))

print(duplicate_variables)

# 468  variables that appear more than once
duplicate_variable_names <- duplicate_variables %>%
  select(variable_name) %>%
  distinct()


# among the duplicate_variables, print counts
duplicate_summary <- duplicate_variables %>%
  filter(duplicated(variable_name) | duplicated(variable_name, fromLast = TRUE)) %>%
...
# View the summary of duplicates
print(duplicate_summary)

# get variables that are not duplicated, so probably need renaming
non_duplicated_variables <- lbx_variables %>%
  anti_join(duplicate_variable_names, by = "variable_name")

print(non_duplicated_variables)
non_duplicated_variables_names = non_duplicated_variables$variable_name

# just getting a list of unique variables
list_of_vars = unique(lbx_variables$variable_name)

# check for missingness
lbxppe_variables <- lbx_variables %>%
  filter(variable_name == "LBXPPE")

# Print the filtered data frame
print(unique(lbx_variables$component))
```
